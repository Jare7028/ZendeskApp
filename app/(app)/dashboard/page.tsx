...
import { getDashboardData, type DashboardView } from "@/lib/metrics/dashboard";
...
const VIEW_CONTENT: Record<DashboardView, { eyebrow: string; title: string; description: string }> = {
  overview: {
    eyebrow: "Portfolio workspace",
    title: "Read the portfolio before drilling into the roster",
    description:
      "Overview keeps intake, service, and staffing as separate lenses so operators can understand what changed before jumping into client or agent detail."
  },
  clients: {
    eyebrow: "Client comparison",
    title: "Compare client portfolios in one operating frame",
    description:
      "Use the same date window and focus controls to compare client demand, service performance, and capacity signals without rebuilding the analysis."
  },
  agents: {
    eyebrow: "Agent analysis",
    title: "Inspect agent workload without losing portfolio context",
    description:
      "Agent analysis preserves the current scope while ranking workload, service outcomes, and utilisation for the people handling the selected ticket set."
  }
};
...
export default async function DashboardPage({
  ...
  const currentView = VIEW_CONTENT[dashboard.view];
  const exportReport = dashboard.view === "overview" ? "overview" : dashboard.view === "clients" ? "clients" : "leaderboard";
        ...
        <>
          <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
            <div className="rounded-[32px] border border-border/60 bg-gradient-to-br from-emerald-50 via-background to-amber-50 p-6 shadow-panel">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">{currentView.eyebrow}</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-tight">{currentView.title}</h2>
              <p className="mt-2 max-w-3xl text-sm text-muted-foreground">{currentView.description}</p>
            </div>
            <div className="flex flex-wrap items-center justify-end gap-3">
              <ExportControls
                csvHref={buildHref("/api/dashboard/export/csv", baseParams, { report: exportReport })}
                pdfHref={buildHref("/api/dashboard/export/pdf", baseParams, { report: exportReport })}
              />
            </div>
          </section>
                  ...
                  <section className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
                    ...
                    <Card className="border-border/60 bg-card/95">
                      <CardHeader>
                        ...
                        <CardDescription>Why the dashboard now separates intake load from reply workload.</CardDescription>
                      </CardHeader>
                      <CardContent className="grid gap-4 text-sm text-muted-foreground">
                        <div className="rounded-2xl bg-muted/40 p-4">
                          <p className="text-xs uppercase tracking-[0.16em]">Intake lens</p>
                          <p className="mt-2 text-foreground">
                            `Tickets/hour` tracks created demand entering the queue inside the selected scope.
                          </p>
                        </div>
                        <div className="rounded-2xl bg-muted/40 p-4">
                          <p className="text-xs uppercase tracking-[0.16em]">Workload lens</p>
                          <p className="mt-2 text-foreground">
                            `Reply workload/hour` tracks the actual handling load once those tickets start moving.
                          </p>
                        </div>
                        <div className="rounded-2xl bg-muted/40 p-4">
